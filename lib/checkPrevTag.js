...
module.exports = async (releases, tags, details, manualPrevTag) => {
  ...
  // If we want to check the branch we are on (currently disabled due to target_commitish not always being the branch name)
  /*
  ...
  */
  // Workaround - simply get the latest release and prerelease
  const latestRelease = releases.all.find(r => !r.prerelease && !r.draft)
  const latestPrerelease = releases.all.find(r => r.prerelease && !r.draft)
